...
function chordNotes(root: number, scale: number[], degree: number, octave: number): number[] {
...
export async function generateMidiBuffer(seed: string): Promise<Buffer> {
  ...
  const root = randInt(48, 60, rng); // C3–C4
  const scaleName = pick(Object.keys(SCALES) as ScaleName[], rng);
  ...
  const rhythmPattern = pick(RHYTHM_PATTERNS, rng);
  const melodyOctave  = randInt(1, 2, rng);   // relative octave for melody
  const chordOctave   = 0;
  const bassOctave    = -1;
